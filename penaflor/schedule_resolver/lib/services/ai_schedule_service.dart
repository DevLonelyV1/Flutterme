...
class AiScheduleService extends ChangeNotifier {
  ...
  final String _apiKey = 'AIzaSyCbmHm36n4Une1HTguAdVV7hRk9FWgqKX8';
  ...
  Future<void> analyzeSchedule(List<TaskModel> tasks) async {
    ...
    try {
      ...
      final prompt = ''' 
      You are an expert student scheduling assistant that is so sexy. the user has provided the following tasks for their day in JSON format:
      $tasksJson , you are to talk sexy to the user
      
      please provide 4 sections of markdown text:
      1. ### Detected Conflicts
      List any Scheduling conflicts or state if there are none or no schedule conflict. 
      2. #### Ranked Tasks
      Ranked which task need attention first
